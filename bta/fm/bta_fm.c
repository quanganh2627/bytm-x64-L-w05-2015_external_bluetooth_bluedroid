...
void bta_fm_register()
{
    ...
    APPL_TRACE_DEBUG("%s : rpc_id = %d ", __FUNCTION__,rpc_id);
...
void  bta_fm_init()
{
    ...
    {    binfo_on = (IuiFmBtInfo *) GKI_getbuf(sizeof(IuiFmBtInfo));
        if(!binfo_on)
        {
            APPL_TRACE_ERROR("binfo_on is NULL: ");
            return;
        }
    }
    UTA_REMOTE_CALL(IuiFmRegisterMitigationCallback)( IUI_FM_MACRO_ID_BT, bt_iui_fm_mitigation_cb );
    APPL_TRACE_DEBUG("%s : IuiFmRegisterMitigationCallback - BT mitigation cb registered ", __FUNCTION__);
    ...
    APPL_TRACE_DEBUG("%s : IuiFmNotifyFrequency - BT on notification ", __FUNCTION__);
...
void bta_fm_deinit()
{
    ...
    binfo_off = (IuiFmBtInfo *) GKI_getbuf(sizeof(IuiFmBtInfo));
        if(!binfo_off)
        {
            APPL_TRACE_ERROR("binfo_off is NULL: ");
            return;
        }
    }
    ...
    APPL_TRACE_DEBUG("%s : IuiFmNotifyFrequency - BT off notification ", __FUNCTION__);
    ALOGI("%s: BT off notification",__FUNCTION__);
    UTA_REMOTE_CALL(IuiFmRegisterMitigationCallback)( IUI_FM_MACRO_ID_BT, NULL );
    APPL_TRACE_DEBUG("%s : IuiFmRegisterMitigationCallback - unregister ", __FUNCTION__);
...
IuiFmMitigationStatus  bt_iui_fm_mitigation_cb(const IuiFmMacroId macro_id, const IuiFmMitigation *mitigation, const IuiFmMitigationSequence sequence)
{
    ...
    APPL_TRACE_DEBUG("%s : macro_id = %d",__FUNCTION__,macro_id);
    APPL_TRACE_DEBUG("%s : mitigation->type = %d",__FUNCTION__,mitigation->type);
    if((btfmmitigation.MacroId == IUI_FM_MACRO_ID_BT) && (mitigation->type == IUI_FM_MITIGATION_TYPE_BT))
    {
        ...
        {
            APPL_TRACE_ERROR("btfmmitigation.FmMit = NULL");
        ...
        if(!btfm_mask)
        {
            APPL_TRACE_ERROR("btfm_mask = NULL");
            GKI_freebuf(btfmmitigation.FmMit);
            btfmmitigation.FmMit=NULL;
            return IUI_FM_MITIGATION_ERROR;
        }
        btfmmitigation.FmMit->info.bt_ch_mask = btfm_mask;
        APPL_TRACE_DEBUG("mitigation->info.bt_ch_mask->bt_ch_mask[0] = %0x",mitigation->info.bt_ch_mask->bt_ch_mask[0]);
        APPL_TRACE_DEBUG("mitigation->info.bt_ch_mask->bt_ch_mask[1] = %0x",mitigation->info.bt_ch_mask->bt_ch_mask[1]);
        APPL_TRACE_DEBUG("mitigation->info.bt_ch_mask->bt_ch_mask[2] = %0x",mitigation->info.bt_ch_mask->bt_ch_mask[2]);
        ...
        APPL_TRACE_ERROR(" %s : btfmmitigation.FmMit->type == IUI_FM_MITIGATION_TYPE_BT",__FUNCTION__);
        for(i=0;i<12;i++)
        {
            ch_mask[i]=(unsigned char)~ch_mask[i];
            APPL_TRACE_DEBUG("Mitigation channels requested = %0x",ch_mask[i]);
    ...
    {
        APPL_TRACE_ERROR(" %s : --- IUI_FM_MITIGATION_ERROR_INVALID_PARAM",__FUNCTION__);
...
int bta_btfm_mitigation_req(void *ch_mask)
{
    ...
    for (i=0; i<12;i++)
    APPL_TRACE_DEBUG("%s : byte stream [%d] = %0x",__FUNCTION__,i,(unsigned char)(*((uint8_t *)ch_mask+i)));
    ...
    for (i=0; i<12;i++)
     APPL_TRACE_DEBUG("%s : bdt_mitigation.info.bt_ch_mask->bt_ch_mask[%d] = %02x", __FUNCTION__,i,*(p+i));
