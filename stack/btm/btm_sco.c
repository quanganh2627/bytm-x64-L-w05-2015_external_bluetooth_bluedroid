...
{
    BTM_TRACE_DEBUG2("%s codec:%d", __func__, codec);
    switch(codec)
    {
                                            ...
                                            void* p_common_setup)
{
    ...
    else
    {
...
#if (BTM_WBS_INCLUDED == TRUE)
        APPL_TRACE_DEBUG2("%s WBS_INCLUDED. esco_codec:%d",__func__, btm_cb.sco_cb.codec_in_use);
        if (btm_cb.devcb.enhanced_hci_sco == TRUE)//&& btm_cb.sco_cb.codec_in_use == 2/*BTA_AG_CODEC_MSBC*/)
        {
            ...
            {
                APPL_TRACE_ERROR1("%s CANNOT SEND btsnd_hcic_config_sync_iface", __func__);
                return BTM_BUSY;
            }
            btm_cb.sco_cb.acl_handle = acl_handle;
#else
            APPL_TRACE_DEBUG1("%s Sending btsnd_hcic_enhanced_setup_sco_conn", __func__);
            ...
            {
                APPL_TRACE_ERROR1("%s CANNOT SEND btm_send_enhanced_setup_sco", __func__);
                return BTM_BUSY;
            }
            else
