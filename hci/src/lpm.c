...
void lpm_init(void)
{
    ...
    if (vendor_interface)
    {
        vendor_interface->op(BT_VND_OP_GET_LPM_IDLE_TIMEOUT, &lpm_param);
...
void lpm_enable(uint8_t turn_on)
{
    ...
    if (turn_on == BT_VND_LPM_ENABLE)
    {
        ...
        if(NULL==vendor_interface)
        {
            BTLPMDBG("%s bt_vnd_if is NULL",__func__);
            return;
        }
        if (vendor_interface->op(BT_VND_OP_LPM_SET_MODE, &lpm_cmd) == -1)
        {
    ...
    else if (turn_on == BT_VND_LPM_DISABLE)
    {
        ...
        if (vendor_interface)
            vendor_interface->op(BT_VND_OP_LPM_SET_RTS_STATE, &state);
...
void lpm_wake_assert(void)
{
    ...
    BTLPMDBG("%s : bt_lpm state %d" ,__func__, bt_lpm_cb.state);
    if (bt_lpm_cb.state != LPM_DISABLED)
    {
...
int lpm_set_device_state(uint8_t source_state, uint8_t next_state)
{
    ...
    BTLPMDBG("%s enter" ,__func__);
    if (bt_lpm_cb.state != LPM_ENABLED || (!vendor_interface)) {
...
again:
    ...
    if ((retval = vendor_interface->op(BT_VND_OP_LPM_SET_DEVICE_STATE, &next_state)) == 0) {
...
void lpm_set_bt_wake_state(uint8_t state)
{
    ...
    if ((bt_lpm_cb.state == LPM_ENABLED))
    {
        if (bt_lpm_cb.bt_wake_state != state)
        {
            ...
            bt_lpm_cb.cts_state = \
                        vendor_interface->op(BT_VND_OP_LPM_SET_BT_WAKE_STATE, &state);
